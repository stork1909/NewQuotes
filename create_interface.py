...
class FormPage(tk.Frame):
    ...
    def load_form(self, form_data):
        ...
        tk.Label(
            self.scroll_frame,
            text=f"{form_data[0]['producer']} — {form_data[0]['product']}",
            font=("Arial", 16)
        ).pack(pady=10)

        # -------------------------
        # Получаем базисы (динамически)
        # -------------------------
        bases = []

        for key in form_data[0].keys():
            if key not in ("producer", "product", "size"):
                bases.append(key)

        # -------------------------
        # Шапка (базисы поставки)
        # -------------------------
        table_frame = tk.Frame(self.scroll_frame)
        table_frame.pack()

        tk.Label(table_frame, text="Размер", borderwidth=1, relief="solid",
                 width=20).grid(row=0, column=0)

        # Функция enumerate() — это своего рода «умный счетчик». Она решает
        # классическую проблему программирования: когда тебе нужно перебирать
        # элементы списка в цикле, но при этом тебе одновременно нужно знать
        # индекс (порядковый номер) текущего элемента.
        for col, base in enumerate(bases, start=1):
            tk.Label(table_frame, text=base, borderwidth=1, relief="solid",
        ...
        for row_index, item in enumerate(form_data, start=1):
            row_entries = {}

            # размер
            tk.Label(table_frame, text=item["size"], borderwidth=1,
                     relief="solid").grid(row=row_index, column=0)

            for col, base in enumerate(bases, start=1):
                entry = tk.Entry(table_frame)
                entry.grid(row=row_index, column=col)
                row_entries[base] = entry
...
with open("samples/forms", "rb") as f:
    forms = pickle.load(f)
